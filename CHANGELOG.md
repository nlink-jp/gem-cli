...
- **darwin code-signature identifier** is now the canonical `gem-cli`.
...
- **`package` Makefile target.** Builds all 5 platforms, signs darwin
  ...
  versioned naming (`gem-cli-vX.Y.Z-<os>-<arch>.zip`), and notarizes
  the darwin zips.

### Changed

- **Darwin releases are now Developer ID signed and Apple-notarized.**
  `gem-cli-v0.3.1-darwin-{amd64,arm64}.zip` carry full Apple Developer
  ...
  users who place `gem-cli` under Dropbox-synced (or any other
...
- **Release zip filenames now embed the version**
  (`gem-cli-vX.Y.Z-<os>-<arch>.zip`), aligning with sibling
  cli-series tools. v0.3.0 assets used version-less names.
